...
class BillingController extends GetxController {
  ...
  int getQueuedQuantity(String productId) {
    int total = 0;
    for (var bill in queuedBills) {
      total += bill.selectedProducts[productId] ?? 0;
    }
    return total;
  }

  int getAvailableStock(Product product) {
    if (product.id.startsWith('quick_')) return 999;
    final queuedQty = getQueuedQuantity(product.id);
    final available = product.count - queuedQty;
    return available < 0 ? 0 : available;
  }

  void increaseQuantity(Product product) {
    final availableStock = getAvailableStock(product);
    ...
    if (availableStock > currentQuantity) {
      ...
      final queuedQty = getQueuedQuantity(product.id);
      final msg = queuedQty > 0
          ? '${product.name}: only $availableStock available ($queuedQty currently on hold)'
  ...
  void resumeQueuedBill(QueuedBill bill) {
    ...
    try {
      ...
      customerPhone = TextEditingController(text: bill.customerPhone);
